...
import { createClient, signInWithPin, signInWithGoogle } from '@/lib/supabase/client'
import { useToast } from '@/components/ui/toast'
import { useAuthStore } from '@/stores'
import { Button, Input, PageLoading, Card } from '@/components/ui/common'
...
function LoginForm() {
    ...
    const { setUser, setRestaurant, loginAsStaff, loginMode, setLoginMode } = useAuthStore()

    const [isLoading, setIsLoading] = useState(false)
    const [ownerForm, setOwnerForm] = useState({ email: '', password: '' })
    const [staffForm, setStaffForm] = useState({ facilityCode: '', pin: '' })
    ...
    const handleStaffLogin = async (e: React.FormEvent) => {
        ...
        try {
            const result = await signInWithPin(staffForm.facilityCode, staffForm.pin)
            if (!result.success || !result.staff || !result.restaurant) throw new Error(result.error || 'Login failed')

            loginAsStaff(result.staff, result.restaurant)
            success('Welcome', `${result.staff.name} is now active`)
            ...
            error('Login Failed', err instanceof Error ? err.message : 'Invalid code or PIN')
        } finally {
            setIsLoading(false)
        }
    }
        ...
        <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            >
                <div className="bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 rounded-[2rem] p-8 shadow-2xl shadow-neutral-200/50 dark:shadow-black/50">
                    ...
                    <Card variant="flat" className="p-1 mb-8 bg-neutral-100 dark:bg-neutral-800/50 rounded-xl flex">
                        <button
                            onClick={() => setLoginMode('owner')}
                            className={cn(
                                "flex-1 py-2 text-xs font-bold uppercase tracking-widest rounded-lg transition-all duration-300",
                                loginMode === 'owner'
                                    ? "bg-white dark:bg-neutral-700 text-orange-600 shadow-sm ring-1 ring-black/5"
                    ...
                    <AnimatePresence mode="wait">
                        {loginMode === 'owner' ? (
                            ...
                            <motion.form
                                key="staff"
                                initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}
                                onSubmit={handleStaffLogin} className="space-y-5"
                            >
                                <Input
                                    label="Restaurant ID"
                                    type="text"
                                    placeholder="delicious-resto"
                                    icon={Building2}
                                    value={staffForm.facilityCode}
                                    onChange={(e: any) => setStaffForm({ ...staffForm, facilityCode: e.target.value })}
                                    className="bg-neutral-50 dark:bg-neutral-800/50"
                                    required
                                />
                                <Input
                                    ...
                                    icon={KeyRound}
                                    value={staffForm.pin}
                                    onChange={(e: any) => setStaffForm({ ...staffForm, pin: e.target.value })}
                                    maxLength={4}
                                    className="bg-neutral-50 dark:bg-neutral-800/50 tracking-[0.5em] font-mono text-center"
                                    required
                                />
                                <Button type="submit" variant="black" className="w-full h-12 uppercase tracking-widest text-xs font-bold rounded-xl" isLoading={isLoading}>
                                    Enter Dashboard
                                </Button>
                            </motion.form>
                        )}
